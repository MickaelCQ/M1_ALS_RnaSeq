...
# Création des combinaisons x/y avec groupement par paquets de 10
combinaisons <- expand.grid(x_var = vars_exp_num, y_var = vars_resp_num, stringsAsFactors = FALSE) %>%
  mutate(group = ceiling(row_number() / 10))  # regroupe par 10 combinaisons

# Fonction pour créer un graphique de scatterplots avec régression par groupe
plot_group <- function(group_num, df, comb_df) {
  comb_subset <- comb_df %>% filter(group == group_num)
  
  # Création du dataframe long pour ggplot
...
# Génération et sauvegarde des graphiques pour chaque groupe
for (i in seq_len(n_groups)) {
  p <- plot_group(i, df_raw, combinaisons)
  print(p)  # Affiche dans le document
  
  # Sauvegarde
  filename <- paste0("Scatterplots_Group_", i, ".png")
  ggsave(filename, p, width = 10, height = 12, dpi = 300)
}
