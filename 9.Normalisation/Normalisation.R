...
expressed_genes <- rowSums(counts_mat) > 0
...
###########################################################################################################
#                                     Etape 7 : Partie normalisation CPM                                  #
###########################################################################################################
Matrice_Final <- read.delim("counts_56genes.tsv", sep = '\t', header = TRUE, check.names = FALSE)
sample_columns <- setdiff(colnames(Matrice_Final), c("Geneid", "Chr", "Start", "End", "Strand", "Length"))
print(sample_columns)

counts_matrix <- as.matrix(sapply(Matrice_Final[, sample_columns], as.numeric))
print(counts_matrix)
